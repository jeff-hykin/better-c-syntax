...
# 
    ...
    grammar[:preprocessor_number_literal] = grammar[:number_literal].reTag(append:"preprocessor")
# 
# #pragma
# 
...
# 
    include_partial = Pattern.new(
        Pattern.new(
            ...
            match: std_space.then(identifier).zeroOrMoreOf(Pattern.new(/\./).then(identifier)).then(std_space).then(@end_of_line.or(lookAheadFor(Pattern.new(/\/\//).or(/;/)))),
            tag_as: "entity.name.other.preprocessor.macro.include"
        ).or(
            # correctly color a lone `#include`
            match: std_space.then(@end_of_line.or(lookAheadFor(Pattern.new(/\/\//).or(/;/)))),
        )
    )
